...
const [
  ...
  ,
  inputPath,
  outputDir = "checkov-report",
  requestedMode = process.env.CHECKOV_REPORT_MODE ?? "detailed",
] = process.argv;
const mode = ["summary", "detailed"].includes(requestedMode)
  ? requestedMode
  : "detailed";
...
if (!inputPath) {
  console.error(
    "Usage: node render.mjs <junit-xml-path> [output-dir] [summary|detailed]",
  );
  process.exit(1);
}

const resolvedInputPath = resolveExistingWorkspacePath(inputPath);
const resolvedOutputDir = resolveWorkspacePath(outputDir);

const xml = await readFile(resolvedInputPath, "utf8");
...
const failedCases = suites
  .flatMap((suite) =>
    suite.cases.map((testCase) => ({ ...testCase, suite: suite.name })),
  )
  .filter((testCase) => testCase.status !== "passed");

const context = readContext();
const status =
  totals.failures > 0 || totals.errors > 0 ? "Action required" : "Passed";
...
const text =
  mode === "summary"
    ? renderSummaryText({ context, totals })
    : renderText({ context, totals, failedCases });
const html =
  mode === "summary"
...
const canonicalOutputDir = resolveExistingWorkspacePath(resolvedOutputDir);
await writeFile(path.join(canonicalOutputDir, "subject.txt"), subject);
await writeFile(path.join(canonicalOutputDir, "body.txt"), text);
await writeFile(path.join(canonicalOutputDir, "body.html"), html);
await writeFile(
  path.join(canonicalOutputDir, "summary.json"),
  JSON.stringify(
    { subject, mode, totals, failedCases: failedCases.length, context },
    null,
    2,
  ),
);

function resolveExistingWorkspacePath(candidate) {
  const canonicalPath = realpathSync(resolveWorkspacePath(candidate));
  assertWorkspacePath(canonicalPath);
  return canonicalPath;
}

function resolveWorkspacePath(candidate) {
  const resolvedPath = path.resolve(workspaceRoot, candidate);
  assertWorkspacePath(resolvedPath);
  return resolvedPath;
}
...
function parseSuites(source) {
  const suiteMatches = [
    ...source.matchAll(/<testsuite\b([^>]*)>([\s\S]*?)<\/testsuite>/g),
  ];
  const suitesToParse =
    suiteMatches.length > 0 ? suiteMatches : [[null, "", source]];
  ...
  return suitesToParse.map((match) => {
    ...
    const cases = [
      ...
      const status = testCaseStatus({ error, failure, skipped });
      ...
      return {
        ...
        details: cleanDetails(
          decodeXml((error ?? failure ?? skipped)?.body ?? ""),
        ),
      };
    });
    ...
    return {
      ...
      failures: numberAttr(
        attrs.failures,
        cases.filter((testCase) => testCase.status === "failure").length,
      ),
      errors: numberAttr(
        attrs.errors,
        cases.filter((testCase) => testCase.status === "error").length,
      ),
      skipped: numberAttr(
        attrs.skipped,
        cases.filter((testCase) => testCase.status === "skipped").length,
      ),
...
function firstTag(source, tagName) {
  const match = source.match(
    new RegExp(
      String.raw`<${tagName}\b([^>]*)>([\s\S]*?)<\/${tagName}>|<${tagName}\b([^>]*)\/>`,
    ),
  );
...
function testCaseStatus({ error, failure, skipped }) {
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      .muted { color: #657083; font-size: 12px; }
...
function renderMetricsHtml(totals) {
  const metricCell = (value, label, border = true) => `<td width="25%" valign="top" style="padding: 18px; border-top: 0; ${border ? "border-right: 1px solid #d9e0ea;" : ""}">
              <div style="font-size: 30px; line-height: 1.1; font-weight: 700; color: #172033;">${value}</div>
              <div style="margin-top: 8px; color: #657083; font-size: 12px; line-height: 1.3;">${label}</div>
            </td>`;

  return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="width: 100%; table-layout: fixed; border-collapse: collapse; border-bottom: 1px solid #d9e0ea;">
          <tr>
            ${metricCell(totals.tests, "Tests")}
            ${metricCell(totals.failures, "Failures")}
            ${metricCell(totals.errors, "Errors")}
            ${metricCell(totals.skipped, "Skipped", false)}
          </tr>
        </table>`;
}
